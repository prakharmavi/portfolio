...
export default function ContactForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok && json?.ok) {
        form.reset();
        toast({ title: "Message sent", description: "Thanks for reaching out — I’ll reply shortly.", kind: "success" });
      } else {
        ...
        else if (code === "turnstile_failed") msg = "Verification failed. Please try again.";
        ...
        else if (code === "mail-not-configured") msg = "Email is not configured. Please use the ‘Talk to me’ button or email me directly.";
        else if (code === "mail-failed") msg = "Could not send email. Please try again later or email me directly.";
        setError(msg);
      }
    } catch {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 gap-3">
          <div>
            <label className="block text-xs text-gray-600" htmlFor="name">Name</label>
            <input id="name" name="name" type="text" required className="mt-1 w-full rounded-2xl border border-gray-200 bg-gray-50 px-3 py-3 text-sm focus:outline-hidden focus:ring-2 focus:ring-gray-300" placeholder="Ada Lovelace (or your dev alias)" />
          </div>
          <div>
            <label className="block text-xs text-gray-600" htmlFor="email">Email</label>
            <input id="email" name="email" type="email" required className="mt-1 w-full rounded-2xl border border-gray-200 bg-gray-50 px-3 py-3 text-sm focus:outline-hidden focus:ring-2 focus:ring-gray-300" placeholder="you@shipping-soon.dev (real email, promise)" />
          </div>
          <div>
            <label className="block text-xs text-gray-600" htmlFor="message">Message</label>
      ...
      <div className="rounded-2xl border border-gray-200 p-4">
        ...
        {!token ? (
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <button onClick={copy} className="inline-flex items-center justify-center rounded-full bg-gray-800 text-white size-9 hover:bg-black focus:outline-hidden" type="button" aria-label={copied ? "Copied" : "Copy email"} title={copied ? "Copied" : "Copy email"}>
            ...
            <a href={`mailto:${EMAIL}`} className="inline-flex items-center justify-center rounded-full border border-gray-200 size-9 text-gray-800 hover:bg-gray-50" aria-label="Open mail app" title="Open mail app">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="size-4" aria-hidden>
