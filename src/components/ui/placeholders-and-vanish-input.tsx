...
export function PlaceholdersAndVanishInput({
  ...
  useEffect(() => {
    const startAnimation = () => {
      if (intervalRef.current || placeholders.length === 0) return;
  ...
  const draw = useCallback(() => {
    ...
    for (let t = 0; t < 800; t++) {
      ...
      for (let n = 0; n < 800; n++) {
        const pixelIndex = rowOffset + 4 * n;
        if (
  ...
  return (
    <form
      className={cn(
        "w-full relative max-w-xl mx-auto bg-white dark:bg-zinc-800 h-12 rounded-[4px] overflow-hidden shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),_0px_1px_0px_0px_rgba(25,28,33,0.02),_0px_0px_0px_1px_rgba(25,28,33,0.08)] transition duration-200",
        value && "bg-gray-50"
      )}
